...
%EvaluateNetwork evaluates network using either training or validation set
...
for iSlope = 1:nSlopes
  ...
  while isTruckOk
    ...
    acceleration = GetAcceleration(slopeAngle(iStep), brakeTemperature(iStep), ...
      maxTemperature, pedalPressure(iStep), truckMass, gear(iStep), engineBreakConstant);
    ...
    [newPedalPressure, requestedGear] = RunFFNN(neuralNetwork,...
      newSpeed/maxSpeed, newSlopeAngle/maxSlope, newTemperature/maxTemperature);
    ...
    if numel(gear) < iStep+1
        pause;
        
    end
  end % end truck run

  fitnessInSlopes(iSlope) = mean(speed)*position(end).^2;
